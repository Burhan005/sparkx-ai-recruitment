...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-[#070A12] text-slate-100 flex flex-col justify-between selection:bg-indigo-600 selection:text-white bg-cyber-grid">
      
      {/* Client Presentation Interactive Quick-Bar */}
      {showDemoGuide && (
        <div className="bg-gradient-to-r from-indigo-950 via-slate-900 to-purple-950 border-b border-indigo-800/40 px-4 py-2 flex flex-col sm:flex-row items-center justify-between text-xs gap-2">
          <div className="flex items-center space-x-2">
            <span className="w-2 h-2 rounded-full bg-cyan-400 animate-ping"></span>
            <span className="font-bold text-indigo-200">Presentation Demo Tour:</span>
            <span className="hidden md:inline text-slate-400">Follow the 6-step end-to-end hackathon workflow</span>
          </div>
          ...
          <button
            ...
            className="text-slate-500 hover:text-slate-300 text-[10px] hidden lg:inline underline"
          >
            Hide Tour
          </button>
        </div>
      ...
      <footer className="border-t border-slate-900/80 bg-[#0B0F19]/90 mt-16 py-6 text-xs text-slate-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-4 text-slate-400">
            ...
            <span>•</span>
            <span className="hover:text-indigo-400 cursor-pointer" onClick={() => setCurrentView('recruiter')}>
              Recruiter Dashboard
            </span>
            <span>•</span>
            <span className="hover:text-indigo-400 cursor-pointer" onClick={() => setCurrentView('proctor')}>
