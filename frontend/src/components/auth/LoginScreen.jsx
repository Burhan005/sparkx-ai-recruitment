...
export default function LoginScreen({ onLogin }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#070A12] text-slate-900 dark:text-slate-100 flex overflow-hidden relative transition-colors duration-300">
      ...
      <div className="absolute top-5 right-5 z-30">
        <button
          type="button"
          onClick={toggleTheme}
          className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl border border-slate-300 dark:border-white/[0.08] bg-white/80 dark:bg-slate-900/80 backdrop-blur-md text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition shadow-sm hover:scale-105 active:scale-95 text-xs font-bold"
      ...
      {/* ─── LEFT PANEL: Branding & Value Pitch ──────────────────────────────── */}
      <div className="hidden lg:flex flex-col justify-between w-1/2 relative overflow-hidden p-12 bg-gradient-to-br from-indigo-900 via-slate-950 to-indigo-950 text-white">
        <div className="absolute top-0 left-0 w-96 h-96 bg-indigo-600/20 rounded-full blur-3xl -translate-x-1/3 -translate-y-1/3" />
        ...
        <div className="relative z-10 flex items-center space-x-2 text-xs text-indigo-200/70">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-12 relative overflow-y-auto bg-slate-50 dark:bg-[#070A12] transition-colors duration-300">
        ...
        <div className="w-full max-w-sm space-y-6">
          ...
          {tab !== 'forgot' ? (
            <div className="flex p-1 rounded-2xl bg-slate-200/90 dark:bg-slate-900 border border-slate-300/80 dark:border-slate-800">
              <button
                type="button"
            ...
            <div className="flex items-center space-x-2">
              <button
                type="button"
                onClick={() => { setTab('signin'); setError(''); setSuccessMsg(''); }}
                className="inline-flex items-center space-x-1.5 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-white transition"
              >
          ...
          {/* Feedback Messages */}
          {error && (
            <div className="p-3 rounded-xl bg-rose-50 dark:bg-rose-950/90 border border-rose-300 dark:border-rose-700/60 text-rose-700 dark:text-rose-300 text-xs font-semibold flex items-center space-x-2 shadow-sm">
              <AlertCircle className="w-4 h-4 shrink-0 text-rose-500" />
              <span>{error}</span>
            </div>
          )}
          {successMsg && (
            <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/90 border border-emerald-300 dark:border-emerald-700/60 text-emerald-700 dark:text-emerald-300 text-xs font-semibold flex items-center space-x-2 shadow-sm">
            ...
            <form onSubmit={tab === 'signin' ? handleSignIn : handleSignUp} className="space-y-4">
              ...
              {tab === 'signup' && (
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-700 dark:text-slate-400 uppercase tracking-wider">Full Name *</label>
                  <input
                    type="text"
                    value={name}
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition shadow-sm"
                  />
                </div>
              )}
              ...
              {tab === 'signup' && (
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-slate-700 dark:text-slate-400 uppercase tracking-wider">Account Type / Role *</label>
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 p-2.5 rounded-xl border text-xs font-bold flex items-center justify-center space-x-2 transition ${
                        role === 'candidate' 
                          ? 'bg-emerald-100 border-emerald-400 text-emerald-800 dark:bg-emerald-950/80 dark:border-emerald-600 dark:text-emerald-300 shadow-sm' 
              ...
              {tab === 'signup' && role === 'recruiter' && (
                <div className="space-y-1.5 p-3 rounded-xl bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800/40">
                  <label className="text-[11px] font-bold text-purple-800 dark:text-purple-300 uppercase tracking-wider flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 font-normal">Security Restricted</span>
                  </label>
                  <input
                    type="password"
                    value={adminCode}
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-purple-300 dark:border-purple-700/50 text-slate-900 dark:text-white text-xs placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:border-purple-500 transition shadow-sm"
                  />
              ...
              {/* Email field */}
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-700 dark:text-slate-400 uppercase tracking-wider">Email Address *</label>
                <input
                  type="email"
                  value={email}
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-[11px] font-bold text-slate-700 dark:text-slate-400 uppercase tracking-wider">Password *</label>
                  {tab === 'signin' && (
                    <button
                      type="button"
                      onClick={() => {
            ...
            <div className="space-y-4">
              ...
              {forgotStep === 1 ? (
                <form onSubmit={handleRequestResetCode} className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-700 dark:text-slate-400 uppercase tracking-wider">Account Email Address *</label>
                    <input
                      type="email"
                      value={forgotEmail}
                  ...
                  <div className="text-center pt-2">
                    <button
                      type="button"
                      onClick={() => { setTab('signin'); setError(''); setSuccessMsg(''); }}
                      className="text-xs text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 transition inline-flex items-center space-x-1.5"
                    >
                      <ArrowLeft className="w-3.5 h-3.5" />
                      <span>Return to Sign In</span>
                    </button>
                ...
                <form onSubmit={handleConfirmResetPassword} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <label className="text-[11px] font-bold text-slate-700 dark:text-slate-400 uppercase tracking-wider">6-Digit Verification Code *</label>
                      <button
                        type="button"
                        onClick={handleResendCode}
                        disabled={loading}
                        className="text-[10px] font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 transition hover:underline disabled:opacity-50"
                      >
                        Resend Code
                      </button>
                    </div>
                    <input
                      ...
                      required
                      placeholder="123456"
                      className="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white font-mono text-center tracking-widest text-lg focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition shadow-sm"
                    />
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-700 dark:text-slate-400 uppercase tracking-wider">New Password *</label>
                    <div className="relative">
                      <input
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-700 dark:text-slate-400 uppercase tracking-wider">Confirm New Password *</label>
                    <input
                      type="password"
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <button
                      ...
                      className="text-xs text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 transition inline-flex items-center space-x-1"
                    >
                      <ArrowLeft className="w-3.5 h-3.5" />
